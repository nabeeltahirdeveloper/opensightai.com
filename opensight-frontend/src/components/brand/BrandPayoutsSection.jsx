...
export default function BrandPayoutsSection() {
  ...
  const [activeTab, setActiveTab] = useState('history'); // 'history' or 'unpaid'
  ...
  const loadPayouts = async (filterParams = {}) => {
    ...
    try {
      ...
      const data = await serverApi.brand.payouts.list(params);
  ...
  const loadUnpaidPayouts = async () => {
    ...
    try {
      ...
      const data = await serverApi.brand.payouts.getUnpaid(params);
