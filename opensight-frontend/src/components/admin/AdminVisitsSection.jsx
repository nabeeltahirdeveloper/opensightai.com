...
export default function AdminVisitsSection() {
  ...
  const loadVisits = async () => {
    setLoading(true);
    try {
      const data = await serverApi.admin.visits.list({ page: String(page), pageSize: '20' });
  ...
  const loadStats = async () => {
    try {
      const data = await serverApi.admin.visits.getStats({ days: String(days) });
      setStats(data);
    } catch (error) {
    ...
    <div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
          ...
          <div className="glass-card p-6 rounded-xl">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-gray-600">VPN/Proxy Blocks</h3>
              <i className="fas fa-ban text-red-500 text-xl"></i>
            </div>
            <p className="text-3xl font-bold text-gray-900">{stats.total_vpn_proxy_blocks || 0}</p>
